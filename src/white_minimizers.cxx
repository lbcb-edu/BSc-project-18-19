...
std::vector<std::tuple<unsigned int, unsigned int, bool>> minimizers(
	...
	if (k > sequence_length) {
		...
		std::cout << "Program doesn't work for k > 16," << std::endl;
		exit(1);
	}
	...
	unsigned int next_window_starting_kmer_reverse_complement = 0;
	unsigned int mask = 0;
	...
	for (int i = 0; (unsigned) i < sequence_length - (window_length - 1 + k) + 1; i++) { // i mi treba kao int zbog pocetne pozicije koja je -1
		...
		//u suprotnom treba samo usporedit sa kranje desnim kmerom prozora
			...
			unsigned int start = i + window_length - 1; //trebat ce posli
			unsigned int end = i + k + window_length - 1; //zbog brzine
